...
export const guest = (() => {
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'The Wedding of Dicky dan Tini',
            dates: `${formatDate('2026-05-26 08:00')}/${formatDate('2025-05-26 09:00')}`,
            details: 'Tanpa mengurangi rasa hormat, kami mengundang Anda untuk berkenan menghadiri acara pernikahan kami. Terima kasih atas perhatian dan doa restu Anda, yang menjadi kebahagiaan serta kehormatan besar bagi kami.',
            location: 'Dusun Karangpung RT 01/08, Desa Pojok, Kec.pulokulon, Kab.Grobogan, Jawa Tengah',
            ctz: config.get('tz'),
        });
    ...
    const pageLoaded = () => {
        ...
        if (token && token.length > 0) {
            // add 1 progress for config (comment removed).
            // before img.load();
            progress.add();
            progress.add();
